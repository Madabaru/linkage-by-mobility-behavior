...
pub fn gen_client_to_target_idx_map<R: Rng, T>(
    ...
    for client in sampled_clients_list.into_iter() {
        let mobility_traces_list = &client_to_vector_map.get(client).unwrap();
        let mobility_trace_len = mobility_traces_list.len();
        // Split click history in 50%/50%
        let split_idx = mobility_trace_len / 2;
        let indices: Vec<usize> = (split_idx..mobility_trace_len).collect();
        let sampled_target_idx = indices
            .into_iter()
            .choose_multiple(rng, target_mobility_trace_sample_size);
...
pub fn gen_client_to_sample_idx_map<R: Rng, T>(
    ...
    for (client, mobility_traces_list) in client_to_vector_map.into_iter() {
        let client = client.clone();
        let mobility_trace_len = mobility_traces_list.len();
        let split_idx = mobility_trace_len / 2;
        let indices: Vec<usize> = (0..split_idx).collect();
        if mobility_trace_sample_size > 0 {
            let sampled_idx = indices
                .into_iter()
                .choose_multiple(rng, mobility_trace_sample_size);
