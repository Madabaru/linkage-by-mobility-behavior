...
pub fn parse_to_frequency(
    config: &Config,
) -> Result<BTreeMap<u32, Vec<FreqMobilityTrace>>, Box<dyn Error>> {
    let mut prev_time: f64 = 0.0;
    let mut prev_client = String::new();
    let mut mobility_trace_len: usize = 0;
    let mut client_id: u32 = 0;

    let mut client_to_freq_map: BTreeMap<u32, Vec<FreqMobilityTrace>> = BTreeMap::new();
    ...
    for result in reader.deserialize() {
        ...
        if prev_client != record.client_id && !prev_client.is_empty() {
            ...
            let prev_mobility_traces_list = client_to_freq_map.get_mut(&client_id).unwrap();
            if !prev_mobility_traces_list.is_empty() {
                if mobility_trace_len < config.min_mobility_trace_len {
                    prev_mobility_traces_list.pop();
                }
            }
            client_id += 1;
        }
        ...
        let mobility_traces_list = client_to_freq_map.get_mut(&client_id).unwrap();

        if mobility_traces_list.is_empty()
            || mobility_trace_len >= config.max_mobility_trace_len
            || record.timestamp - prev_time >= config.delay_limit
        {
            if !mobility_traces_list.is_empty() {
                if mobility_trace_len < config.min_mobility_trace_len
                    || mobility_traces_list.last().unwrap().end_time
                        - mobility_traces_list.last().unwrap().start_time
                        > config.max_mobility_trace_duration
                {
                    mobility_traces_list.pop();
                }
            }
            ...
            mobility_traces_list.push(mobility_trace);
            mobility_trace_len = 0;
        }

        let current_mobility_trace = mobility_traces_list.last_mut().unwrap();
        ...
        current_mobility_trace.hour[hour_index] += 1;
        current_mobility_trace.day[day_index] += 1;
        current_mobility_trace.end_time = record.timestamp;

        *current_mobility_trace
            .speed
    ...
    client_to_freq_map.retain(|_, value| value.len() >= config.min_num_mobility_traces);
    log::info!(
...
pub fn parse_to_sequence(
    config: &Config,
) -> Result<BTreeMap<u32, Vec<SeqMobilityTrace>>, Box<dyn Error>> {
    let mut prev_time: f64 = 0.0;
    let mut prev_client = String::new();
    let mut mobility_trace_len: usize = 0;
    let mut client_id: u32 = 0;

    let mut client_to_seq_map: BTreeMap<u32, Vec<SeqMobilityTrace>> = BTreeMap::new();
    let mut reader = csv::Reader::from_path(&config.path)?;
    ...
    for result in reader.deserialize() {
        ...
        if prev_client != record.client_id && !prev_client.is_empty() {
            let prev_mobility_traces_list = client_to_seq_map.get_mut(&client_id).unwrap();
            if !prev_mobility_traces_list.is_empty() {
                if mobility_trace_len < config.min_mobility_trace_len {
                    prev_mobility_traces_list.pop();
                }
        ...
        let mobility_traces_list = client_to_seq_map.get_mut(&client_id).unwrap();
            ...
            || record.timestamp - prev_time >= config.delay_limit
        {
            ...
            let mobility_trace = SeqMobilityTrace {
        ...
        current_mobility_trace.hour.push(datetime.hour());
        current_mobility_trace.day = datetime.weekday().num_days_from_monday();
        current_mobility_trace.end_time = record.timestamp;
        ...
        current_mobility_trace.location_code.push(
    ...
    client_to_seq_map.retain(|_, value| value.len() >= config.min_num_mobility_traces);
    log::info!(
        "Number of clients after filtering: {:?}",
        client_to_seq_map.keys().len()
    );
